...
struct clip_layer {
    ...
    bool has_deepstack() const { return deepstack_fc1_w != nullptr; }
};
...
struct clip_model {
    ...
    bool audio_has_avgpool() const { return proj_type == PROJECTOR_TYPE_QWEN2A || proj_type == PROJECTOR_TYPE_VOXTRAL; }
...
struct clip_ctx {
    ...
    projector_type proj_type() const { return model.proj_type; }
};
...
struct clip_graph {
    ...
    ggml_cgraph * build_siglip() {
        ...
        ggml_tensor * cur = build_vit(inp, n_patches, NORM_TYPE_NORMAL, hparams.ffn_op, learned_pos_embd, nullptr);
        ...
        if (ctx->proj_type() == PROJECTOR_TYPE_GEMMA3) {
            ...
            cur = build_ffn(cur, model.mm_0_w, model.mm_0_b, nullptr, nullptr, model.mm_1_w, model.mm_1_b,
                            hparams.ffn_op, -1);
    ...
    ggml_cgraph * build_qwen3vl() {
        ...
        learned_pos_embd = ggml_cont_4d(ctx0, learned_pos_embd, n_embd * 2, n_patches_x / 2, n_patches_y, batch_size);
        learned_pos_embd =
            ggml_reshape_4d(ctx0, learned_pos_embd, n_embd * 2, n_patches_x / 2, 2, batch_size * (n_patches_y / 2));
        learned_pos_embd = ggml_permute(ctx0, learned_pos_embd, 0, 2, 1, 3);
        learned_pos_embd = ggml_cont_3d(ctx0, learned_pos_embd, n_embd, n_patches_x * n_patches_y, batch_size);
        ...
        const int     merge_factor =
            hparams.n_merge > 1 ? hparams.n_merge * hparams.n_merge : 4;  // default 2x2=4 for qwen3vl
        ...
        for (int il = 0; il < n_layer; il++) {
            ...
            // self-attention
            {
                ...
                ggml_tensor * Qcur =
                    ggml_view_3d(ctx0, cur, d_head, n_head, n_pos, d_head * sizeof(float), cur->nb[1], 0);
                ggml_tensor * Kcur = ggml_view_3d(ctx0, cur, d_head, n_head, n_pos, d_head * sizeof(float), cur->nb[1],
                                                  n_embd * sizeof(float));
                ggml_tensor * Vcur = ggml_view_3d(ctx0, cur, d_head, n_head, n_pos, d_head * sizeof(float), cur->nb[1],
            ...
            if (layer.has_deepstack()) {
                ggml_tensor * feat =
                    ggml_reshape_3d(ctx0, cur, n_embd * merge_factor, n_pos / merge_factor, batch_size);
                feat = build_norm(feat, layer.deepstack_norm_w, layer.deepstack_norm_b, norm_t, eps, il);
                feat = build_ffn(feat, layer.deepstack_fc1_w, layer.deepstack_fc1_b, nullptr, nullptr,
                                 layer.deepstack_fc2_w, layer.deepstack_fc2_b, ffn_op_type::FFN_GELU, il);
        ...
        embeddings = build_ffn(embeddings, model.mm_0_w, model.mm_0_b, nullptr, nullptr, model.mm_1_w, model.mm_1_b,
    ...
    ggml_cgraph * build_llama4() {
        ...
        {
            ggml_tensor * kernel = ggml_reshape_4d(ctx0, model.patch_embeddings_0, patch_size, patch_size, 3, n_embd);
        ...
        ggml_tensor * cur = build_vit(inp, n_pos, NORM_TYPE_NORMAL, hparams.ffn_op, model.position_embeddings, add_pos);
        ...
        // https://github.com/huggingface/transformers/blob/2932f318a20d9e54cc7aea052e040164d85de7d6/src/transformers/models/llama4/modeling_llama4.py#L1151
        {
            ...
            cur = ggml_reshape_4d(ctx0, cur, n_embd * scale_factor, n_patches_x / scale_factor, n_patches_y, bsz);
            cur = ggml_permute(ctx0, cur, 0, 2, 1, 3);
            cur = ggml_cont_4d(ctx0, cur, n_embd * scale_factor * scale_factor, n_patches_x / scale_factor,
                               n_patches_y / scale_factor, bsz);
            //cur = ggml_permute(ctx0, cur, 0, 2, 1, 3);
            // flatten to 2D
            cur =
                ggml_cont_2d(ctx0, cur, n_embd * scale_factor * scale_factor, n_patches / scale_factor / scale_factor);
    ...
    ggml_cgraph * build_kimivl() {
        ...
        ggml_tensor * inp = build_inp();
        ggml_tensor * cur = build_vit(inp, n_patches, NORM_TYPE_NORMAL, hparams.ffn_op, learned_pos_embd, add_pos);
...

        {
            ...
            cur              = ggml_view_2d(ctx0, cur, n_embd, cur->ne[1] * scale_factor * scale_factor,
                                            ggml_row_size(cur->type, n_embd), 0);
            cur              = ggml_norm(ctx0, cur, 1e-5);  // default nn.LayerNorm
            cur              = ggml_mul(ctx0, cur, model.mm_input_norm_w);
            cur              = ggml_add(ctx0, cur, model.mm_input_norm_b);
            cur              = ggml_view_2d(ctx0, cur, proj_inp_dim, cur->ne[1] / scale_factor / scale_factor,
    ...
    ggml_cgraph * build_whisper_enc() {
        ...
        ggml_tensor * pos_embd_selected =
            ggml_view_2d(ctx0, model.position_embeddings, model.position_embeddings->ne[0], n_pos,
                         model.position_embeddings->nb[1], 0);
        ggml_tensor * cur = build_vit(inp, n_pos, NORM_TYPE_NORMAL, hparams.ffn_op, pos_embd_selected, nullptr);
        ...
        if (model.audio_has_stack_frames()) {
            ...
            cur = ggml_view_2d(ctx0, cur, stride, padded_len / stride, ggml_row_size(cur->type, stride), 0);
    ...
    ggml_cgraph * build_cogvlm() {
        ...
        for (int il = 0; il < n_layer; il++) {
            ...
            ggml_tensor * Qcur = ggml_view_3d(ctx0, cur, d_head, n_head, n_pos, d_head * sizeof(float), cur->nb[1], 0);
            ggml_tensor * Kcur = ggml_view_3d(ctx0, cur, d_head, n_head, n_pos, d_head * sizeof(float), cur->nb[1],
                                              n_embd * sizeof(float));
            ggml_tensor * Vcur = ggml_view_3d(ctx0, cur, d_head, n_head, n_pos, d_head * sizeof(float), cur->nb[1],
                                              2 * n_embd * sizeof(float));
        ...
        ggml_tensor * cur = ggml_view_2d(ctx0, inpL, n_embd, n_patches, ggml_row_size(inpL->type, n_embd), 0);
  ...
  private:
    ...
    ggml_tensor * resize_position_embeddings() {
        ...
        pos_embd =
            ggml_reshape_3d(ctx0, pos_embd, n_embd, n_per_side, n_per_side);  // -> (n_embd, n_per_side, n_per_side)
    ...
    ggml_tensor * build_vit(ggml_tensor *                                                   inp,
                            int64_t                                                         n_pos,
                            norm_type                                                       norm_t,
                            ffn_op_type                                                     ffn_t,
    ...
    ggml_tensor * build_norm(ggml_tensor * cur,
                             ggml_tensor * mw,
                             ggml_tensor * mb,
                             norm_type     type,
                             float         norm_eps,
    ...
    static ggml_tensor * build_rope_2d(ggml_context * ctx0,
                                       ggml_tensor *  cur,
        ...
        const float freq_scale_odd = interleave_freq ? std::pow(freq_base, (float) -2 / n_dim) : 1.0;
    ...
    ggml_tensor * build_patch_merge_permute(ggml_tensor * cur, int scale_factor) {
        ...
        // unshuffle w
        cur =
            ggml_cont_3d(ctx0, cur, n_embd * scale_factor * scale_factor, height / scale_factor, width / scale_factor);
        cur = ggml_permute(ctx0, cur, 0, 2, 1, 3);
    ...
    void load_hparams(clip_model & model, clip_modality modality) {
        ...
        std::string proj_type;
        {
            ...
            if (model.proj_type == PROJECTOR_TYPE_QWEN25O) {
                model.proj_type = modality == CLIP_MODALITY_VISION ? PROJECTOR_TYPE_QWEN25VL : PROJECTOR_TYPE_QWEN2A;
            }
        }
        ...
        // other hparams
        {
            ...
            switch (model.proj_type) {
                ...
                case PROJECTOR_TYPE_QWEN3VL:
                    {
                        ...
                        get_u32(KEY_WIN_ATTN_PATTERN, hparams.n_wa_pattern,
                                model.proj_type == PROJECTOR_TYPE_QWEN25VL);  // only 2.5 requires it
                        ...
                        hparams.set_warmup_n_tokens(256);  // avoid OOM on warmup
                        const int warn_min_pixels =
                            1024 * hparams.n_merge * hparams.n_merge * hparams.patch_size * hparams.patch_size;
                        if (hparams.image_min_pixels < warn_min_pixels) {
                            LOG_WRN(
                                "%s: Qwen-VL models require at minimum 1024 image tokens to function correctly on "
                                "grounding tasks\n",
                                __func__);
                ...
                case PROJECTOR_TYPE_VOXTRAL:
                    {
                        bool require_stack =
                            model.proj_type == PROJECTOR_TYPE_ULTRAVOX || model.proj_type == PROJECTOR_TYPE_VOXTRAL;
            ...
            if (is_vision) {
                ...
                if (hparams.image_min_pixels > 0) {
                    LOG_INF("%s: image_min_pixels:   %d%s\n", __func__, hparams.image_min_pixels,
                            hparams.custom_image_min_tokens > 0 ? " (custom value)" : "");
                }
                if (hparams.image_max_pixels > 0) {
                    LOG_INF("%s: image_max_pixels:   %d%s\n", __func__, hparams.image_max_pixels,
                            hparams.custom_image_max_tokens > 0 ? " (custom value)" : "");
                }
            } else if (is_audio) {
                LOG_INF("\n--- audio hparams ---\n");
            ...
            LOG_INF("%s: metadata size:      %.2f MiB\n", __func__,
                    ggml_get_mem_size(ctx_meta.get()) / 1024.0 / 1024.0);
        }
    }
            ...
            case PROJECTOR_TYPE_LDP:
                {
                    ...
                    model.mm_model_block_1_block_0_0_w =
                        get_tensor(string_format(TN_MVLM_PROJ_BLOCK, 1, 0, "0.weight"));
                    model.mm_model_block_1_block_0_1_w =
                        get_tensor(string_format(TN_MVLM_PROJ_BLOCK, 1, 0, "1.weight"));
                    model.mm_model_block_1_block_0_1_b = get_tensor(string_format(TN_MVLM_PROJ_BLOCK, 1, 0, "1.bias"));
                    model.mm_model_block_1_block_1_fc1_w =
                        get_tensor(string_format(TN_MVLM_PROJ_BLOCK, 1, 1, "fc1.weight"));
                    model.mm_model_block_1_block_1_fc1_b =
                        get_tensor(string_format(TN_MVLM_PROJ_BLOCK, 1, 1, "fc1.bias"));
                    model.mm_model_block_1_block_1_fc2_w =
                        get_tensor(string_format(TN_MVLM_PROJ_BLOCK, 1, 1, "fc2.weight"));
                    model.mm_model_block_1_block_1_fc2_b =
                        get_tensor(string_format(TN_MVLM_PROJ_BLOCK, 1, 1, "fc2.bias"));
                    model.mm_model_block_1_block_2_0_w =
                        get_tensor(string_format(TN_MVLM_PROJ_BLOCK, 1, 2, "0.weight"));
                    model.mm_model_block_1_block_2_1_w =
                        get_tensor(string_format(TN_MVLM_PROJ_BLOCK, 1, 2, "1.weight"));
                    model.mm_model_block_1_block_2_1_b = get_tensor(string_format(TN_MVLM_PROJ_BLOCK, 1, 2, "1.bias"));
                    model.mm_model_block_2_block_0_0_w =
                        get_tensor(string_format(TN_MVLM_PROJ_BLOCK, 2, 0, "0.weight"));
                    model.mm_model_block_2_block_0_1_w =
                        get_tensor(string_format(TN_MVLM_PROJ_BLOCK, 2, 0, "1.weight"));
                    model.mm_model_block_2_block_0_1_b = get_tensor(string_format(TN_MVLM_PROJ_BLOCK, 2, 0, "1.bias"));
                    model.mm_model_block_2_block_1_fc1_w =
                        get_tensor(string_format(TN_MVLM_PROJ_BLOCK, 2, 1, "fc1.weight"));
                    model.mm_model_block_2_block_1_fc1_b =
                        get_tensor(string_format(TN_MVLM_PROJ_BLOCK, 2, 1, "fc1.bias"));
                    model.mm_model_block_2_block_1_fc2_w =
                        get_tensor(string_format(TN_MVLM_PROJ_BLOCK, 2, 1, "fc2.weight"));
                    model.mm_model_block_2_block_1_fc2_b =
                        get_tensor(string_format(TN_MVLM_PROJ_BLOCK, 2, 1, "fc2.bias"));
                    model.mm_model_block_2_block_2_0_w =
                        get_tensor(string_format(TN_MVLM_PROJ_BLOCK, 2, 2, "0.weight"));
                    model.mm_model_block_2_block_2_1_w =
                        get_tensor(string_format(TN_MVLM_PROJ_BLOCK, 2, 2, "1.weight"));
    ...
    static void warmup(clip_ctx & ctx_clip) {
        ...
        if (ctx_clip.flash_attn_type == CLIP_FLASH_ATTN_TYPE_AUTO) {
            ...
            if (!info.fattn && info.fattn_op) {
                ...
                static auto print_shape = [](const char * fn, const char * name, ggml_tensor * t) {
                    LOG_WRN("%s:   %s: type = %s, ne = [%d %d %d %d], nb = [%d %d %d %d]\n", fn, name,
                            ggml_type_name(t->type), t->ne[0], t->ne[1], t->ne[2], t->ne[3], t->nb[0], t->nb[1],
                            t->nb[2], t->nb[3]);
                };
            ...
            if (!info.fattn && ctx_clip.flash_attn_type == CLIP_FLASH_ATTN_TYPE_ENABLED) {
                LOG_WRN(
                    "%s: flash attention is not supported by the current backend; falling back to CPU (performance "
                    "will be degraded)\n",
                    __func__);
            }
        }
        ...
        if (ctx_clip.backend && ctx_clip.backend != ctx_clip.backend_cpu) {
            ...
            if (!unsupported_ops.empty()) {
                ...
                for (const auto & op : unsupported_ops) {
                    LOG_WRN("%s: %16s: type = %s, ne = [%d %d %d %d]\n", __func__, ggml_op_name(op.op->op),
                            ggml_type_name(op.op->type), op.op->ne[0], op.op->ne[1], op.op->ne[2], op.op->ne[3]);
                }
...
static void normalize_image_u8_to_f32(const clip_image_u8 & src,
                                      clip_image_f32 &      dst,
                                      const float           mean[3],
                                      const float           std[3]) {
    dst.nx = src.nx;
...
struct img_tool {
    ...
    static void resize(const clip_image_u8 &   src,
                       clip_image_u8 &         dst,
    ...
    static clip_image_size calc_size_preserved_ratio(const clip_image_size & inp_size,
...
struct llava_uhd {
    ...
    static slice_instructions get_slice_instructions(struct clip_ctx * ctx, const clip_image_size & original_size) {
        ...
        if (has_pinpoints) {
            ...
            auto refine_size =
                llava_uhd::select_best_resolution(original_size, ctx->model.hparams.image_res_candidates);
            res.overview_size   = clip_image_size{ slice_size, slice_size };
            ...
            LOG_DBG("%s: original size: %d x %d, overview size: %d x %d, refined size: %d x %d\n", __func__,
                    original_width, original_height, res.overview_size.width, res.overview_size.height,
            ...
            for (int y = 0; y < refine_size.height; y += slice_size) {
                for (int x = 0; x < refine_size.width; x += slice_size) {
                    ...
                    LOG_DBG("%s: slice %d: x=%d, y=%d, size=%dx%d\n", __func__, (int) res.slices.size() - 1, slice.x,
                            slice.y, slice.size.width, slice.size.height);
                }
            }
...

        {
            ...
            LOG_DBG("%s: original size: %d x %d, overview size: %d x %d, refined size: %d x %d, grid size: %d x %d\n",
                    __func__, original_width, original_height, res.overview_size.width, res.overview_size.height,
                    res.refined_size.width, res.refined_size.height, res.grid_size.width, res.grid_size.height);
  ...
  private:
    static clip_image_size get_best_resize(const clip_image_size & original_size,
    ...
    static clip_image_size resize_maintain_aspect_ratio(const clip_image_size & orig,
                                                        const clip_image_size & target_max) {
    ...
    static clip_image_size select_best_resolution(const clip_image_size &              original_size,
        ...
        for (const clip_image_size & candidate : possible_resolutions) {
            ...
            int  effective_resolution =
                std::min(target_size.width * target_size.height, original_size.width * original_size.height);
            int wasted_area = (candidate.width * candidate.height) - effective_resolution;

            if (effective_resolution > max_effective_resolution ||
                (effective_resolution == max_effective_resolution && wasted_area < min_wasted_area)) {
                max_effective_resolution = effective_resolution;
                min_wasted_area          = wasted_area;
                best_fit                 = candidate;
            }

            LOG_DBG("%s: candidate: %d x %d, target: %d x %d, wasted: %d, effective: %d\n", __func__, candidate.width,
                    candidate.height, target_size.width, target_size.height, wasted_area, effective_resolution);
        }
    ...
    static clip_image_size get_refine_size(const clip_image_size & original_size,
        ...
        case PROJECTOR_TYPE_QWEN3VL:
            {
                ...
                const clip_image_size new_size = img_tool::calc_size_preserved_ratio(
                    original_size, params.patch_size * 2, params.image_min_pixels, params.image_max_pixels);
                ...
                for (int y = 0; y < refined_size.height; y += params.image_size) {
                    for (int x = 0; x < refined_size.width; x += params.image_size) {
                        ...
                        instructions.slices.push_back(llava_uhd::slice_coordinates{
                            ...
                            /* y    */ y,
                            /* size */
                            clip_image_size{ std::min(params.image_size, refined_size.width - x),
        ...
        case PROJECTOR_TYPE_LIGHTONOCR:
            {
                ...
                const clip_image_size target_size = img_tool::calc_size_preserved_ratio(
                    original_size, params.patch_size * cur_merge, params.image_min_pixels, params.image_max_pixels);
        ...
        case PROJECTOR_TYPE_KIMIVL:
            {
                ...
                const clip_image_size target_size =
                    img_tool::calc_size_preserved_ratio(original_size, params.patch_size * params.n_merge,
        ...
        case PROJECTOR_TYPE_COGVLM:  // TODO @ngxson : is this correct for cogvlm?
            {
                ...
                if (params.image_res_candidates.empty()) {  // pad_to_square
                    ...
                    img_tool::resize(*img, *temp, clip_image_size{ params.image_size, params.image_size },
                                     img_tool::RESIZE_ALGO_BILINEAR, true, pad_color);
        ...
        case PROJECTOR_TYPE_LIGHTONOCR:
            {
                ...
                int n_patches_x = img->nx / patch_size / (n_merge > 1 ? n_merge : 1);
                int n_patches_y = img->ny / patch_size / (n_merge > 1 ? n_merge : 1);
                if (ctx->model.token_embd_img_break) {
                    n_patches =
                        n_patches_y * n_patches_x + n_patches_y - 1;  // + one [IMG_BREAK] per row, except the last row
                } else {
...
static std::vector<std::vector<std::vector<float>>> get_2d_sincos_pos_embed_from_grid(
    int                                                  embed_dim,
    const std::vector<std::vector<std::vector<float>>> & grid) {
    assert(embed_dim % 2 == 0);
...
bool clip_image_batch_encode(clip_ctx *                   ctx,
                             const int                    n_threads,
